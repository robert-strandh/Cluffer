...
operation completes, what happens to \textit{cursor} and other cursors
at the same position as \textit{cursor} depends on the class of the
cursor and of the line to which \textit{cursor} is attached.  The
\texttt{standard-line} implementation provides two kinds of cursors,
namely \emph{left-sticky} cursors and \emph{right-sticky} cursors.
For such an implementation, after this operation completes, any
left-sticky cursor located at the same position as \textit{cursor}
will be located before \textit{item}, and any right-sticky cursor
located at the same position as \textit{cursor} will be located after
\textit{item}.
